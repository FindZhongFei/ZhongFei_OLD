    ...
    public void onSuccess(JSONObject result) {
        ...
        try
        {
            ...
            if(this.isSuccess || this.successMessage.equals ("success"))
            {
                ...
                else if(requestType.equals("requestChats"))
                {
...
//                        int lastMessageIndex = everyMessageArray.length() - 1;
                        int unreadCount = 0;

//                        messagesArray = retrievedData.getJSONArray("messages");
//                        for(int indexUnreadMessages = 0; indexUnreadMessages < everyMessageArray.length(); indexUnreadMessages++)
//                        {
//
//                        }
...
//                        ChatComposeActivity.mMessagesList.add(indexPartners, eachChatMessages);
                    ...
                    ChatFragment.chatRoomArrayList.addAll(chatsArrayList);
...
//                USER REQUESTS
                ...
                else if(requestType.equals("requestUserProfilePicture"))
                {
                    ...
                    Log.d(TAG, "onSuccess: ChatUser Logo from result " + userProfile.getUserProfilePicture());
                    byte[] decodedLogo = Base64.decode(result.get("imageFile").toString(), Base64.DEFAULT);
...
//                    JSONArray chats = new JSONArray(String.valueOf(result.get("messages")));
//
//                    ChatList chatRoom;
//                    ArrayList<ChatList> chatsArrayList = new ArrayList<>();
//
//                    for (int i = 0; i < chats.length(); i++) {
//                        JSONObject retrievedData = chats.getJSONObject(i);
//
//                        chatRoom = new ChatList(
//                                retrievedData.getString("senderName"),
//                                retrievedData.getString("senderName"),
//                                retrievedData.getString("senderName"),
//                                retrievedData.getString("senderName"),
//                                retrievedData.getInt("id")
//                        );
//                        chatsArrayList.add(i, chatRoom);
//                    }
//
//                    ChatFragment.chatRoomArrayList.addAll(chatsArrayList);
//                    ChatFragment.mAdapter = new ChatListAdapter(context, chatsArrayList);
